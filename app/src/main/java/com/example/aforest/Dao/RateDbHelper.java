...
public class RateDbHelper extends SQLiteOpenHelper {
	...
	String table_name = "rate";//用于存放用户信息的表名
	final String Tag = "sqlop";//写入日志时用的标签
	String db_name;//数据库文件名，在构造函数中传入

	public RateDbHelper(Context context, String name, CursorFactory factory, int version) {
	...
	public void createtable() {
		...
		try {
			...
			String sql = "Create table if not exists " + table_name +
					"(username Varchar not null, title Varchar not null,content Varchar,id Varchar);";
			db.execSQL(sql);//执行SQL语句
			Log.v(Tag, "Create rateTable...");
		} catch (Exception e) {
	...
	//插入新评论
	public void insert(String UId, String title,String content) {
		try {
			...
			String sql = "Insert into " + table_name +
					" (username,title,content) values('" + UId + "','" + title+ "','" + content + "');";//构建插入用户的SQL
	...
	//更新记录，修改密码及用户资料时使用
	public void update(String username, String title,String titleRe, String content) {
		try {
			db = context.openOrCreateDatabase(db_name, Context.MODE_PRIVATE, null);

			String sql = "update " + table_name +
					" set title='" + titleRe + "',content='" + content +"' where username='" + username +"'and title='" +title + "';";
			db.execSQL(sql);
	...
	//查询某一条记录，登录显示用户信息时使用
	public Cursor select(String username,String title) {
		try {
			db = context.openOrCreateDatabase(db_name, Context.MODE_PRIVATE, null);
			String sql = "username='" + username + "and title='"+title +"'";
