...
public class contactDbHelper extends SQLiteOpenHelper{
	...
	/*增删改查本全为protected方法，此处为不同包调用改为public，实为不规范操作*/
		...
		String table_name = "contact";//用于存放用户信息的表名
		...
		public void createtable() {
			...
			try {
				...
				String sql = "Create table if not exists " + table_name +
						"(username Varchar not null, phone Varchar not null,sex Varchar);";
		...
		//插入新评论
		public void insert(String username, String phone,String sex) {
			try {
				...
				String sql = "Insert into " + table_name +
						" (username,phone,sex) values('" + username + "','" + phone+ "','" + sex + "');";//构建插入用户的SQL
	...
	//更新记录，修改用户资料时使用
	public void update(String username,String usernameRe, String sex,String phone) {
		try {
			db = context.openOrCreateDatabase(db_name, Context.MODE_PRIVATE, null);

			String sql = "update " + table_name +
					" set username='" + usernameRe + "',sex='" + sex +"',phone='" + phone +  "' where username='" + username + "';";
			db.execSQL(sql);
		...
		//查询某一条记录，登录显示用户信息时使用
		public Cursor select(String username,String phone) {
			try {
				db = context.openOrCreateDatabase(db_name, Context.MODE_PRIVATE, null);
				String sql = "username='" + username + "'and phone='" + phone  +  "';";
